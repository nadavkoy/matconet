...
class ClientHandler(threading.Thread):
        ...
        if entrance_type == 'register':  # if the client chose to register (connecting as a new user)
            ...
            if self.database.add_user(client_username, client_password) is False:  # if details are wrong
                ...
                self.sock.send(self.encrypt_message(
                    SUCCESSFUL_ENTRY + SONG_SEPARATOR + MESSAGES + SONG_SEPARATOR + SEPARATOR2.join(ALL_SONG_LIST),
                    self.key))
            ...
            if self.done_logging_in and SONG_LIST:
                ...
                for song in SONG_LIST:  # iterating the song list, in order to send the songs.
                    ...
                    if song == SONG_LIST[0]:  # if its the first song:
                        self.song_socket.send(song_title + SONG_SEPARATOR + str(
                            time_passed()) + SONG_SEPARATOR)  # sending song title along with the time that has
    ...
    def get_song_list(self, song_list_message):
        ...
        song_list = song_list.split(SONG_SEPARATOR)  # extracting the song list
    ...
    def run(self):
        ...
        while True:
            ...
            elif message_from_client.startswith(FINAL_SCORE):
                message_from_client = message_from_client.split(SONG_SEPARATOR)
                client_score = message_from_client[1]

                global ALL_SCORES

                ALL_SCORES.update({client_score: self.username})

                winner = get_winner()

                self.sock.send(self.encrypt_message(FINAL_SCORE + SONG_SEPARATOR + winner, self.key))
